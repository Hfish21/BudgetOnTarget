...
- **Storage provider** (`src/components/storage-provider.tsx`): React context managing file open/save, IndexedDB auto-persist (2s debounce), Google Drive open/save + conflict resolution (persists a small Drive ref — never the token — in IndexedDB), and a `dataVersion` counter components watch to re-fetch after store mutations
